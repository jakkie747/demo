 ...
 */
export const deleteTeacherUser = onCall({ cors: true }, async (request) => {
  if (!request.auth) {
    ...
    if (teacherDocToDelete.exists()) {
        ...
        if (teacherData?.photo && teacherData.photo.includes('firebasestorage.googleapis.com')) {
            try {
                ...
                const fileRef = storage.bucket().file(new URL(teacherData.photo).pathname.split('/').slice(3).join('/'));
                await fileRef.delete();
                logger.log(`Successfully deleted photo for user ${uidToDelete} at path ${fileRef.name}`);
            } catch (storageError: any) {
                // If the file doesn't exist, we can ignore the error. Otherwise, log it.
                if (storageError.code !== 404) { // GCS uses 404 for object not found
