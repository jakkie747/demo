...
import { getAfterschoolChildren, deleteAfterschoolChild } from "@/services/afterschoolService";
...
export default function AfterschoolChildrenPage() {
  ...
  const fetchChildren = useCallback(async () => {
    setIsLoading(true);
    try {
      const fetchedChildren = await getAfterschoolChildren();
      ...
      if (error.message.includes("index")) {
        ...
        toast({ variant: "destructive", title: "Error", description: error.message || "Could not fetch afterschool children." });
      }
      setChildren([]);
    } finally {
      setIsLoading(false);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await deleteAfterschoolChild(deletingChild.id);
      toast({
                ...
                {isLoading ? (
                    ...
                    <TableRow>
                        <TableCell colSpan={4} className="h-24 text-center">
                            No afterschool children registered yet.
                        </TableCell>
                    </TableRow>
                ) : (
                    <TooltipProvider>
                        ...
                        <TableCell>
                            <div className="flex gap-1 justify-end">
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button asChild variant="ghost" size="icon">
                                            <Link href={`/admin/dashboard/afterschool/${child.id}/reports`}><FileText className="h-4 w-4" /></Link>
                                        </Button>
                                    </TooltipTrigger>
                                ...
                                <Tooltip>
                                    <TooltipTrigger asChild>
                                        <Button asChild variant="ghost" size="icon">
                                            <Link href={`/admin/dashboard/afterschool/${child.id}/edit`}><Edit className="h-4 w-4" /></Link>
                                        </Button>
                                    </TooltipTrigger>
